...
export default function Content({ setRefresh, refresh, credentialSelected, setCredentialSelected }: ContentProps) {
  const [showModal, setShowModal] = useState(false);
  const [fields, setFields] = useState<IFields[]>([]);
  const [fav, setFav] = useState<boolean>(credentialSelected.favorite);
      ...
      {credentialSelected._id ? (
        <div className='basis-2/3 flex flex-col justify-between m-4 bg-gray-100 items-center rounded-lg border-2 border-amber-300 shadow-lg'>
          <div className="mt-2 pl-4 pr-4 flex justify-between w-full">
            <svg onClick={() => {
              deleteCredential(credentialSelected._id, credentialSelected.userId || 0);
              setRefresh((prevRefresh) => !prevRefresh);
            ...
            <p className='bg-green-500 pt-1 pb-1 pr-2 pl-2 rounded-xl'>{credentialSelected?.category}</p>
            {fav ? (
              <>
                <svg onClick={() => {
                  toggleFavorite(credentialSelected._id, false, credentialSelected.userId || 0);
              ...
              <>
                <svg onClick={() => {
                  toggleFavorite(credentialSelected._id, true, credentialSelected.userId || 0);
