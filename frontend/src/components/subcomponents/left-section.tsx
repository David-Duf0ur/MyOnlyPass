...
export default function LeftSection({ }: ILeftSectionProps) {
  ...
  const { filter, setFilter, setFilterCategory } = useContext(FilterContext);
  ...
  return (
    <>
      <div style={{ backgroundImage: `url(${bg2})` }}
        ...
        <ol className='mt-[100px]'>
          ...
          <ol>
            ...
            <li onClick={() => setShowModalStock(true)} className='flex content-center items-center gap-1 ml-4 mt-2 cursor-pointer hover:underline'>
      ...
      {showModalStock && (
        <Stock setShowModalStock={setShowModalStock} />
      )
      }
    </>
  )
